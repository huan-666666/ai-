...
try:
    ...
    from statsmodels.tsa import stattools as tsastats
...
class FontUtils:
    FONT_FAMILIES = ['Microsoft YaHei', 'SimHei', 'PingFang SC', 'Microsoft YaHei UI', 'Segoe UI', 'Arial', 'Tahoma',
                     'Verdana', 'Helvetica']
...
class DataUtils:
    ...
    def parse_raw_data(raw_text):
        ...
        try:
            ...
            if date_match_cn:
                date_str = date_match_cn.group(1) + "年" + date_match_cn.group(2).zfill(2) + "月" + date_match_cn.group(
                    3).zfill(2) + "日"
            else:
                date_match_dash = re.search(r'(\d{4})-(\d{1,2})-(\d{1,2})', raw_text)
                if date_match_dash:
                    date_str = date_match_dash.group(1) + "年" + date_match_dash.group(2).zfill(
                        2) + "月" + date_match_dash.group(3).zfill(2) + "日"
            if date_str is None:
...
class PredictionAlgorithms:
    ...
    def _prepare_tensorflow_model(self):
        ...
        try:
            ...
            self.tf_predictions = {n + 1: float(p) for n, p in
                                   enumerate(self.tf_lstm_model.predict(X_last, verbose=0)[0])}
    ...
    def _tf_fc_classifier(self, X_seq, y_seq):
        ...
        try:
            ...
            self.tf_fc_predictions = {n + 1: float(p) for n, p in
                                      enumerate(self.tf_fc_model.predict(X_last_flat, verbose=0)[0])}
        except:
            ...
            for i in range(len(self.data) - 5):
                ...
                for j in range(5):
                    ...
                    for n in self.data[i + j].get('numbers', []):
                        if 1 <= n <= 49:
                            one_hot[n - 1] = 1.0
                    seq.append(torch.tensor(one_hot))
                X_pt.append(torch.stack(seq).unsqueeze(0))
                next_nums = self.data[i].get('numbers', [])
                y_pt.append(torch.tensor([1.0 if n in next_nums else 0.0 for n in range(1, 50)]))
            if len(X_pt) < 10:
                self.pt_lstm_preds = {}
                return
            X_pt = torch.cat(X_pt, dim=0)
            y_pt = torch.stack(y_pt)
    ...
    def _pt_autoencoder(self):
        """PyTorch AutoEncoder特征降维"""
        try:
            X = torch.tensor(self.sklearn_features_scaled, dtype=torch.float32)
                ...
                try:
                    if sm is not None and HAS_STATSMODELS:
                        ...
                        if len(series) > 1:
                            ...
                            var = np.var(series)
                            if var > 0:
                                autocorr = np.sum((np.array(series[:-1]) - mean) * (np.array(series[1:]) - mean)) / (
                                            len(series) * var)
    ...
    def _calculate_moving_avg(self):
        ...
        for window in [5, 10, 20]:
            ...
            for i in range(len(self.data)):
                ...
                rate = sum(
                    1 for r in subset if r.get('numbers') and any(num in r['numbers'] for num in range(1, 50))) / len(
                    subset) if subset else 0
                rates.append(rate)
            for num in range(1, 50):
    ...
    def comprehensive_recommendation(self, count=6):
        ...
        # SciPy插值预测加成
        scipy_bonus = self._scipy_interp_bonus() if hasattr(self,
                                                            'scipy_interp_func') and self.scipy_interp_func else {}
    ...
    def hot_cold_algorithm(self, count=6):
        ...
        # SciPy卷积平滑趋势
        scipy_smooth_bonus = self._scipy_hot_smooth() if hasattr(self,
                                                                 'scipy_smoothed') and self.scipy_smoothed is not None else {}
    ...
    def big_small_algorithm(self, count=6):
        ...
        # 融合预测
        predicted_bigs = int(
            round(np_predicted_bigs * 0.25 + rf_predicted_bigs * 0.3 + tf_bigs * 0.3 + scipy_pred * 0.15))
        ...
        if range_counts[0] < range_counts[3]:
            small_candidates = sorted(small_candidates, key=lambda x: LotteryConfig.get_range_index(x) == 0,
                                      reverse=True)
    ...
    def missing_value_analysis(self, count=6):
        ...
        scipy_interp_bonus = self._scipy_missing_interp() if hasattr(self,
                                                                     'scipy_interp_func') and self.scipy_interp_func else {}
    ...
    def range_distribution_algorithm(self, count=6):
        ...
        scipy_range_bonus = self._scipy_range_interp() if hasattr(self,
                                                                  'scipy_interp_func') and self.scipy_interp_func else {}
        ...
        range_weights = {}
        for i in range(5):
            chi2_weight = max(0, (expected - recent_range_freq.iloc[i]) / expected) * 5 + (
                        total - recent_range_freq.iloc[i]) / total
    ...
    def roulette_selection(self, count=6):
        ...
        # NumPy矩阵运算融合权重
        combined_weights = np.array(weights) * 0.3 + np.array(lr_weights) * 200 * 0.2 + np.array(
            tf_weights) * 100 * 0.25 + np.array(pt_weights) * 100 * 0.25
        ...
        if HAS_TORCH and self.device is not None:
            weights_tensor = torch.tensor(combined_weights, dtype=torch.float32, device=self.device)
    ...
    def historical_similarity(self, count=6):
        ...
        pca_based_sim = self._pca_historical_similarity() if hasattr(self,
                                                                     'sklearn_features') and self.sklearn_features is not None else []

        # TensorFlow AutoEncoder降维相似度
        tf_sim = self._tf_autoencoder_similarity() if hasattr(self,
                                                              'tf_encoded') and self.tf_encoded is not None else []
    ...
    def mystical_algorithm(self, count=6):
        ...
        tf_random_weights = self._tf_mystical_weights() if hasattr(self,
                                                                   'tf_predictions') and self.tf_predictions else np.ones(
            49)
        ...
        if HAS_TORCH and self.device is not None:
            base_weights = torch.tensor(base_weights_np, dtype=torch.float32, device=self.device)
            probs = torch.softmax(base_weights / 0.7, dim=0)
            probs_np = probs.cpu().numpy()
        else:
            # NumPy替代实现
    ...
    def pytorch_deep_learning_algorithm(self, count=6):
        ...
        try:
            ...
            for temp in temp_schedule:
                scaled = logits / temp
                softmax_probs = torch.softmax(scaled, dim=0).numpy()
    ...
    def _pytorch_fallback(self, count=6):
        ...
        try:
            for num in range(1, 50):
                ...
                scores[
                    num] = base_score * 0.3 + miss_score * 100 * 0.25 + freq_score * 100 * 0.2 + trend_score * 0.15 + corr_s * 100 * 0.1
        except:
            for num in range(1, 50):
                scores[num] = self._get_weighted_score(num)
...
class NumberButton(QPushButton):
    ...
    def _apply_color(self):
        ...
        self.setStyleSheet(
            "QPushButton { background-color: #FFFFFF; color: " + colors['text'] + "; border: 2px solid " + colors[
                'border'] + "; border-radius: 8px; font-weight: bold; font-size: 16px; }" +
...
class StatisticsChart(QWidget):
    ...
    def plot_distribution(self, data, title="分布统计"):
        ...
        wedges, texts, autotexts = ax.pie(values, labels=labels, colors=colors[:len(labels)], autopct='%1.1f%%',
                                          startangle=90)
        for text in texts:
...
class LotteryPredictionWindow(QMainWindow):
    ...
    def _predict_seventh_all(self):
        ...
        result += "尾数分析：大尾" + str(big_tail_count) + "次 小尾" + str(
            small_tail_count) + "次 -> 预判：" + tail_pred + "\n\n"
        result += "综合预判结果：\n"
            ...
            for record in self.historical_data:
                for num in record.get('numbers', []):
                    if num > 24:
                        ...
                        small_count += 1
            self.main_chart_widget.plot_distribution({'大号(25-49)': big_count, '小号(1-24)': small_count},
                                                     "大小分布统计")
            ...
            for record in self.historical_data:
                for num in record.get('numbers', []):
                    if LotteryConfig.is_red(num):
                        ...
                        green_count += 1
            self.main_chart_widget.plot_distribution({'红色': red_count, '蓝色': blue_count, '绿色': green_count},
                                                     "颜色分布统计")
    ...
    def _update_stylesheet(self):
        ...
        stylesheet = """
            ...
            QPushButton { background-color: #FFFFFF; color: #000000; border: 2px solid #DDDDDD; border-radius: 6px; padding: 6px 16px; font-size: """ + str(
            small_font_size) + """px; min-height: 30px; }
            ...
            QTabBar::tab { background-color: #FFFFFF; color: #000000; border: 1px solid #DDDDDD; padding: 8px 20px; margin-right: 2px; font-size: """ + str(
            small_font_size) + """px; }
            QTabBar::tab:selected { background-color: #3498DB; color: white; border-color: #3498DB; }
            QTabBar::tab:hover { background-color: #E8F4FC; }
            QTextEdit, QLineEdit { background-color: #FFFFFF; color: #000000; border: 2px solid #DDDDDD; border-radius: 4px; padding: 8px; font-size: """ + str(
            font_size) + """px; }
            QTextEdit:focus, QLineEdit:focus { border-color: #3498DB; }
            QComboBox { background-color: #FFFFFF; color: #000000; border: 2px solid #DDDDDD; border-radius: 4px; padding: 6px 12px; font-size: """ + str(
            small_font_size) + """px; min-height: 28px; }
            ...
            QTableWidget { background-color: #FFFFFF; color: #000000; border: 1px solid #DDDDDD; border-radius: 4px; font-size: """ + str(
            small_font_size) + """px; }
            ...
            QStatusBar { background-color: #FFFFFF; color: #333333; border-top: 1px solid #DDDDDD; font-size: """ + str(
            small_font_size) + """px; }
    ...
    def _on_import_clicked(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "导入数据", "",
    ...
    def _on_export_clicked(self):
        if not self.historical_data:
            ...
            return
        file_path, _ = QFileDialog.getSaveFileName(self, "导出数据", "lottery_data_export.json",
    ...
    def _on_add_data_clicked(self):
        text, ok = QInputDialog.getMultiLineText(self, "添加数据",
    ...
    def _on_batch_delete_clicked(self):
        ...
        if not selected_rows:
            QMessageBox.information(self, "提示", "请先选择要删除的记录")
            return
        reply = QMessageBox.question(self, "确认批量删除", "确定要删除选中的 " + str(len(selected_rows)) + " 条记录吗？",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
    ...
    def _on_batch_import_clicked(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(self, "批量导入", "",
